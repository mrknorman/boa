...
plt.imshow(dataset_x[0][0].reshape(input_dims, input_dims))
...
for epoch in range(epochs):
    ...
    for batch_features in train_loader:
        ...
        l_discrete    = torch.divide(torch.sum(distanceTo0(mask)), torch.sum(mask))
        l_mask        = torch.mean(mask)
                        
        model.n_reconstruct = 10
        model.n_mask        = 0.1
        model.n_discrete    = 0.0
        ...
        train_loss = l_reconstruct + l_mask# + l_discrete
...
outputs, mask = model.cpu()(dataset_x[0][0].view(-1, input_dims**2).cpu())
