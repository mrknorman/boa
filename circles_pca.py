...
class AE(nn.Module):
    ...
    def forward(self, features):
        ...
        code_mult = torch.mul(code,mask)
        #code_mult = torch.mul(code_mult, self.rank)
        
        activation = self.decoder_hidden_layer(code_mult)
        activation = torch.relu(activation)
        activation = self.decoder_output_layer(activation)
        reconstructed = torch.sigmoid(activation)
        return reconstructed, mask, code
...
"""
dataset_x = torchvision.datasets.MNIST('./data/', train=True, download=True,
...
train_loader = torch.utils.data.DataLoader(
    dataset_x, batch_size=1024, shuffle=True, num_workers=4, pin_memory=True
)
...
for epoch in range(epochs):
    ...
    for batch_features in train_loader:
        ...
        outputs, mask, code = model(batch_features)
        
        feature_spaces = code.T
        V = torch.abs(torch.cov(feature_spaces) - torch.eye(latent_dimensions).to(device)*torch.cov(feature_spaces))
        
        covariance_loss = torch.sum(V)*0.001

        if first == True:
            sum_feature_spaces = torch.sum(feature_spaces, axis = 1)
            first = False
        else:
            sum_feature_spaces += torch.sum(feature_spaces, axis = 1)
        ...
        train_loss = reconstruction_loss + covariance_loss # + mask_size_loss + mask_entropy_loss
    ...
    print(sum_feature_spaces)
    # display the epoch training loss
    print("epoch : {}/{}, loss = {:.6f}, reconstruction_loss = {:.6f}, mask_size_loss = {:.6f}, mask_entropy_loss = {:.6f}, covariance_loss = {:.6f}, size =  {:.6f}".format(epoch + 1, epochs, loss, reconstruction_loss_total, mask_size_loss_total, mask_entropy_loss_total, covariance_loss, mask_size))
...
plt.imshow(v_hard_activation(sum_feature_spaces.cpu().detach()).reshape(1,torch.numel(mask)))
...
outputs, mask, code = model.cpu()(dataset_x[0].view(-1, 784).cpu())
