...
class AE(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.discriminator_output_layer = nn.Linear(
            in_features=kwargs["latent_dims"], out_features=2
        )
        
        self.mask_initial = torch.round(torch.rand((kwargs["latent_dims"],)))
    ...
    def decode(multi):
        ...
        activation = self.decoder_hidden_layer(multi)
    ...
    def forward(self, features):
        ...
        mask  = self.mask
        multi = torch.mul(code,mask)
        
        reconstructed = decode(multi)
        return reconstructed, mask, code #torch.abs(self.mask)
...
model = AE(latent_dims = 100, input_shape=784).to(device)
...
dataset_x = datasets.MNIST(root='./data', train=True, download=True, transform=None);
...
for epoch in range(epochs):
    ...
    for batch_features in train_loader:
        ...
        nn.MSELoss()(outputs, batch_features)
...
plt.imshow(model.cpu().mask.detach()).reshape(1,torch.numel(mask))#
