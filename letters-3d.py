...
try:
    ...
    for i in tqdm(range(15, 84-15-1)):
        for j in range(15, 84-15-1):
            ...
            for c in np.arange(0.1, 1, colours):
                datasets.append( circle(template, (i, j), 15, c))
                labels.append(label)
    
    datasets = np.array(datasets)
    labels = np.array(labels)
    
    np.save("circles3d", datasets, allow_pickle = True)
    np.save("labels3d", labels, allow_pickle = True)
...
for epoch in range(epochs):
    ...
    print("epoch : {}/{}, loss = {:.6f}, r_loss = {:.6f}, m_loss = {:.6f}, size =  {:.6f}".format(epoch + 1, epochs, loss, r_loss, m_loss, mask_size))
