...
class AE(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.discriminator_hidden_layer_2 = nn.Linear(
            in_features=kwargs["latent_dims"], out_features=kwargs["latent_dims"]
        )
        self.discriminator_output_layer = nn.Linear(
            in_features=kwargs["latent_dims"], out_features=1
        )
    ...
    def forward(self, features):
        ...
        activation = torch.relu(activation)
        activation = self.discriminator_hidden_layer_2(activation)
        code = torch.relu(code)
        
        mask = v_hard_activation(self.mask)
        code = torch.mul(code,mask)
        
        activation = self.discriminator_output_layer(code)
...
for epoch in range(epochs):
    ...
    for batch_features in train_loader:
        ...
        outputs, mask, fake_class, real_class = model(batch_features)
        ...
        g_loss = nn.BCELoss()(torch.zeros_like(), fake)
                
        d_loss = torch.mean(torch.log(fake_class) + torch.log(1 - real_class));
